<script lang="ts">
  import PlanetaryResonance from "../animations/PlanetaryResonance.svelte";
  ...
  function prevClient() {
    currentClientIndex =
      (currentClientIndex - 1 + clients.length) % clients.length;
  }
    ...
    <div class="flex-1 md:text-start md:ml-10">
      ...
      <div class="flex md:flex-row flex-col my-5">
        <div class="pillBox flex-1 flex">
          <div
            class=" flex text-black items-center justify-center w-8 h-8 bg-gray-200 rounded-full"
          >
        ...
        <div class="flex flex-row md:mx-5 space-x-5 justify-center items-center">
          <button
            ...
            class="{isLeftDisabled
              ? 'bg-zinc-800'
              : 'bg-gray-200'} -rotate-90 flex md:flex-1 flex-row items-center justify-center w-8 h-8 my-5"
          >
