<script lang="ts">
  import { services } from '$lib/content/site';
  import { page } from '$app/stores';
  import GridOneCol from '$lib/components/pageSections/GridOneCol.svelte';
  import type { ActionData } from './$types';
  import { enhance } from '$app/forms';

  //   export let form: ActionData;
  export let service;
  export let formTitle;

  let pronoun = 'I';
  let amare = 'am';
  let submitting = false;
  let submitResult;

  function onSubmit() {
    submitting = true;
    return async ({ result, update }) => {
      submitting = false;
      submitResult = result.data;
      setTimeout(() => {
        submitResult = '';
      }, 5000);

      await update({ reset: false });
    };
  }

  // TODO: Optimize all the page containers
  // TODO: Hero does not stretch if page is a container?!
  $: {
    // FIXME: Are all these reactive things necessary?
    amare = pronoun === 'I' ? 'am' : 'are';
  }
</script>

<GridOneCol cssClass="mb-8">
  <div class="sectionText md:w-4/5">
    {#if submitResult?.status === 200}
      <div class="sectionText items-start space-y-4 bg-surface-dark-800">
        Thank you for your interest in {service?.title}. We have sent an email to you for
        confirmation. Please check your spam folder to ensure you received it. We will reach out to
        you soon.
      </div>
    {:else if submitResult?.message}
      <div class="sectionText items-start space-y-4 bg-surface-dark-800">
        {submitResult.message}
      </div>
    {:else}
      <form method="POST" use:enhance={onSubmit} action="/contact">
        <div class="sectionText items-start space-y-4 bg-surface-dark-800">
          <h3 class="text-center">{formTitle ?? ''}</h3>
          <select class="select select-ghost h-full" name="pronoun" bind:value={pronoun}>
            <option selected>I</option>
            <option>We</option>
          </select>
          {amare}
          <input
            type="text"
            class="input h-full"
            name="entity"
            placeholder="<name/group>"
            required
          />
          ,
          <input
            type="text"
            class="input h-full"
            name="designation"
            placeholder="<role/designation>"
            required
          />
          at
          <input
            type="text"
            class="input h-full"
            name="business"
            placeholder="<business name>"
            required
          />.
          {pronoun}
          {amare} interested in
          <input
            type="text"
            class="input w-1/2 h-full"
            name="requirements"
            value={service?.title ?? ''}
            placeholder="<requirements>"
            readonly={service}
            required
          />
          , and may be reached at
          <input
            type="email"
            class="input h-full"
            name="email"
            placeholder="<professional email>"
            required
          />.
          <div class="flex flex-row justify-center">
            <button type="submit" disabled={submitting} class="btn btn-secondary self-end">
              {#if submitting}
                <svg
                  width="20"
                  height="20"
                  fill="currentColor"
                  class="mr-2 animate-spin"
                  viewBox="0 0 1792 1792"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M526 1394q0 53-37.5 90.5t-90.5 37.5q-52 0-90-38t-38-90q0-53 37.5-90.5t90.5-37.5 90.5 37.5 37.5 90.5zm498 206q0 53-37.5 90.5t-90.5 37.5-90.5-37.5-37.5-90.5 37.5-90.5 90.5-37.5 90.5 37.5 37.5 90.5zm-704-704q0 53-37.5 90.5t-90.5 37.5-90.5-37.5-37.5-90.5 37.5-90.5 90.5-37.5 90.5 37.5 37.5 90.5zm1202 498q0 52-38 90t-90 38q-53 0-90.5-37.5t-37.5-90.5 37.5-90.5 90.5-37.5 90.5 37.5 37.5 90.5zm-964-996q0 66-47 113t-113 47-113-47-47-113 47-113 113-47 113 47 47 113zm1170 498q0 53-37.5 90.5t-90.5 37.5-90.5-37.5-37.5-90.5 37.5-90.5 90.5-37.5 90.5 37.5 37.5 90.5zm-640-704q0 80-56 136t-136 56-136-56-56-136 56-136 136-56 136 56 56 136zm530 206q0 93-66 158.5t-158 65.5q-93 0-158.5-65.5t-65.5-158.5q0-92 65.5-158t158.5-66q92 0 158 66t66 158z"
                  >
                  </path>
                </svg>
                Submitting
              {:else}
                Request Consulting Session
              {/if}
            </button>
          </div>
        </div>
      </form>
    {/if}
  </div>
</GridOneCol>
